...
Buckets: 5 theory-driven industry groups (imported from _equation_utils)
'''
...
from _equation_utils import NACE_TO_BUCKET, BUCKET_NAMES
...
def main():
    ...
    # Bucket assignment (imported from _equation_utils, single source of truth)
    ...
    # Drop aggregate manufacturing rows (no bucket assignment)
    ...
    for b in sorted(BUCKET_NAMES.keys()):
        n = len(df_out[df_out["bucket"] == b])
        pct = n / len(df_out) * 100
        logger.info(f"  Bucket {b} ({BUCKET_NAMES[b]}): {n} rows ({pct:.1f}%)")
