'''
Equation 4: Industry-by-Industry Coordination Moderation

For EACH industry separately:
  ln(Hours) = β₁ ln(Robots) + β₂[ln(Robots)×HighCoord] + controls + FE

Runs separate regressions per NACE industry to reveal heterogeneity in how
coordination moderates the robot–hours relationship across industries.

Usage (run from project root):
  python code/6-equation4-industry-heterogeneity-coordination.py 1   # diagnostics
  python code/6-equation4-industry-heterogeneity-coordination.py 2   # diagnostics + model
'''
...
def step_model(df_raw: pd.DataFrame) -> None:
    ...
    req = ["ln_hours", "ln_robots_lag1", "ln_va", "ln_cap", "coord", "nace_r2_code"]
    ...
    if "high_coord" not in df.columns:
        logger.warning("high_coord missing; computing from coord. Re-run 2-cleaning-data.py.")
    ...
    logger.info(f"\n{BAR}")
    logger.info(f"  Running industry-by-industry coordination moderation...")
    ...
    for ind in industries:
        ...
        if df_ind["high_coord"].nunique() < 2:
            msg = f"  Skipping {ind}: no variation in high_coord"
            logger.warning(msg)
            skipped.append({"industry": ind, "reason": "no variation in moderator", "n_obs": n_obs})
            continue

        formula = (
            f"ln_hours ~ ln_robots_lag1 + ln_robots_lag1:high_coord"
        ...
        beta_interaction = res.params.get("ln_robots_lag1:high_coord", np.nan)
        se_interaction = res.std_errors.get("ln_robots_lag1:high_coord", np.nan)
        pval_interaction = res.pvalues.get("ln_robots_lag1:high_coord", np.nan)
        ...
        logger.info(
            f"  {ind}: beta_coord={sign}{beta_interaction:.4f} (p={pval_interaction:.3f}){stars}, "
        ...
        results.append({
            ...
            "beta_robot_lowcoord": beta_robot,
            "beta_robot_highcoord": beta_robot + beta_interaction,
            "beta_coordination": beta_interaction,
            "se_coordination": se_interaction,
            "pval_coordination": pval_interaction,
            "ci_lower": ci_lower,
            "ci_upper": ci_upper,
    ...
    n_positive = sum(1 for r in results if r["beta_coordination"] >= 0)
    n_negative = sum(1 for r in results if r["beta_coordination"] < 0)
    n_sig = sum(1 for r in results if r["significant"])

    logger.info(f"  Completed: {n_completed}/{n_total} industries")
    logger.info(f"    Positive coordination effects: {n_positive}")
    logger.info(f"    Negative coordination effects: {n_negative}")
    ...
    if results:
        ...
        results_df = results_df.sort_values("pval_coordination")
        csv_path = OUTPUT_PATH / "industry_by_industry_coordination.csv"
        results_df.to_csv(csv_path, index=False)
        logger.info(f"\n  Results CSV saved: {csv_path}")

        txt_path = OUTPUT_PATH / "equation4_industry_coordination_summary.txt"
        with open(txt_path, "w") as f:
            f.write("Equation 4: Industry-by-Industry Coordination Moderation\n")
            f.write("=" * 80 + "\n")
            f.write(f"Model: ln_hours ~ ln_robots_lag1 + ln_robots_lag1:high_coord + controls + FE\n")
            f.write(f"Each row = separate regression for one NACE industry\n")
            f.write("-" * 80 + "\n\n")
            f.write(f"{'Industry':<12} {'N_obs':>6} {'N_ctry':>6} {'beta_coord':>12} "
            ...
            for _, row in results_df.iterrows():
                ...
                f.write(
                    ...
                    f"{row['beta_coordination']:>12.4f} {row['se_coordination']:>10.4f} "
                    f"{row['pval_coordination']:>10.4f} "
            ...
            f.write(f"  Positive coordination effects: {n_positive}\n")
            f.write(f"  Negative coordination effects: {n_negative}\n")
        ...
        if len(sig_df) > 0:
            for _, row in sig_df.head(10).iterrows():
                sign = "+" if row["beta_coordination"] >= 0 else ""
                logger.info(
                    f"    {row['industry']}: {sign}{row['beta_coordination']:.4f} "
                    f"(p={row['pval_coordination']:.3f})"
                )
        else:
